...
public abstract class DBOperateHelper extends
		GenericDaoImpl<Object, ColumnBean> implements IDbOperator {
	...
	public List<ColumnBean> selectDBTableColumnList(String tableName) {
		...
		for (ColumnBean columnBean : columns) {
			columnBean.setJavaType(getJavaType(columnBean.getData_type(),
					columnBean.getNumeric_precision(),
					columnBean.getNumeric_scale()));
		}
		return columns;
	}
