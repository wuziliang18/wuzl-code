...
public class ServiceCodeCreate extends AbstractFileCreateCodeOperator {
	...
	public String doCreateNewCode(ParamBean paramBean) {
		...
		String projectPath = paramBean
				.getPath(Constants.Service.SERVICE_INTERFACES_PROJECT_PATH);
		/* 3.获取验证包名 */
		...
		code.append("public interface " + serviceName
				+ " extends GenericService<" + beanName + ", "
				+ paramBean.getPriColumn().getJavaType() + "> {"
				+ LINE_SEPARATOR + LINE_SEPARATOR);
		code.append("}");
