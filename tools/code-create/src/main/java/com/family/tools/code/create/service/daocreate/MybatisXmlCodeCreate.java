...
public class MybatisXmlCodeCreate extends AbstractXmlFileCreateCodeOperator {
	...
	public Document doCreateNewXmlCode(ParamBean paramBean) {
		...
		priWhere.append("=#{");
		priWhere.append(ConfigUtil.changeUnderLineToHump(priColumn
				.getColumn_name()));
		priWhere.append("}");
		...
		for (int i = 0; i < columns.size(); i++) {// 解析列
			...
			String beanColumnName = ConfigUtil
					.changeUnderLineToHump(columnName);
		...
		deleteElement
				.setText("delete from " + tableName + " where " + priWhere);
		...
		updateElement.setText("update " + tableName + " set " + updateSql
				+ " where " + priWhere);
	...
	public static void main(String[] args) {
		...
		IDbOperator mysqlDbOperator = context.getBean("mysqlDbOperator",
				IDbOperator.class);
		String beanPackage = "com.family.tools.code.create.service.daocreate";
		String beanName = "TestBean";
		String tableName = "admin_log";
		List<ColumnBean> columns = mysqlDbOperator
		...
		findByFilterElement.setAttribute("parameterType", "java.util.Map");
		findByFilterElement.setAttribute("resultType", beanName);
		findByFilterElement.setText(selectSql.toString()
				+ " <where> ${DynamicSQL} ${SortSQL} </where>");
		whereElement = new Element("where");// where
