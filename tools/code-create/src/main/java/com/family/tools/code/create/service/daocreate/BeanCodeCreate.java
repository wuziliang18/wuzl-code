...
public class BeanCodeCreate extends AbstractFileCreateCodeOperator {
	...
	public String doCreateNewCode(ParamBean paramBean) {
		...
		code.append("public class " + beanName + " implements Serializable {"
				+ LINE_SEPARATOR);
		...
		for (ColumnBean columnBean : columns) {
			...
			String columnNameHump = ConfigUtil
					.changeUnderLineToHump(columnName);
			String columnNameHumpFont = ConfigUtil
					.changeUnderLineToHumpAndFontUp(columnName);
			...
			setGetMethod.append("\tpublic void set" + columnNameHumpFont + "("
					+ javaType + " " + columnNameHump + ") {" + LINE_SEPARATOR);
