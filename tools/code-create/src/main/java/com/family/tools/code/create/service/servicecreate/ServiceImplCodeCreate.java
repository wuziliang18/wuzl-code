...
public class ServiceImplCodeCreate extends AbstractFileCreateCodeOperator {
	...
	public String doCreateNewCode(ParamBean paramBean) {
		...
		/* 2.获取service工程的绝对路径 */
		String projectPath = paramBean
				.getPath(Constants.Service.SERVICE_IMPL_PROJECT_PATH);
		/* 3.获取验证包名 */
		String daoPackagePath = paramBean
				.getPath(Constants.Dao.DAO_PACKAGE_PATH);
		String packagePath = paramBean
				.getPath(Constants.Service.SERVICE_PACKAGE_PATH);
		...
		/* 4.生成service名称 */
		String daoName = paramBean.getPrefix()
				+ ConfigUtil.changeUnderLineToHumpAndFontUp(tableName) + "Dao";
		String serviceName = paramBean.getPrefix()
				+ ConfigUtil.changeUnderLineToHumpAndFontUp(tableName)
				+ "Service";
		String serviceImplName = paramBean.getPrefix()
				+ ConfigUtil.changeUnderLineToHumpAndFontUp(tableName)
				+ "ServiceImpl";
		String beanName = paramBean.getPrefix()
				+ ConfigUtil.changeUnderLineToHumpAndFontUp(tableName);
		/* 5.设置文件名 */
		...
		code.append("import " + daoPackagePath + "." + daoName + ";"
				+ LINE_SEPARATOR + LINE_SEPARATOR);
		code.append("public class " + serviceImplName
				+ " extends GenericServiceImpl<" + beanName + ", "
				+ paramBean.getPriColumn().getJavaType() + ">  implements "
				+ serviceName + "{" + LINE_SEPARATOR);
		code.append("\tprivate " + daoName + " dao;" + LINE_SEPARATOR);
		code.append("\tpublic " + serviceImplName + "(" + daoName + " dao) {"
				+ LINE_SEPARATOR);
