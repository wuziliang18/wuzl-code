...
public class ReadWriteDataSource extends AbstractDataSource implements
		InitializingBean {
	private static Log log = LogFactory.getLog(ReadWriteDataSource.class);
	...
	public void afterPropertiesSet() throws Exception {
		if (writeDataSource == null) {
			throw new IllegalArgumentException("writeDataSource不可以为空");
		}
		if (CollectionUtils.isEmpty(readDataSourceMap)) {
			throw new IllegalArgumentException("readDataSourceMap不可以为空");
		}
	...
	private DataSource determineDataSource() {
		...
		if (ReadWriteDataSourceDecision.isChoiceNone()) {
			log.debug("没有配置数据源，默认选择writeDataSource");
			return writeDataSource;
		}
	...
	private DataSource determineReadDataSource() {
		...
		log.debug(String.format("当前数据是%s", dataSourceName));
